...
class OrderService
{
     ...
     * @return array
     */
    private function decideProfitLoss(int|float $paid, int|float $taxTotal, int|float $productBuyingSubtotal = 0, int|float $cartSubtotal = 0, Order $order = null): array
    {
        if ($order) {
            $productBuyingSubtotal = 0;
        ...
        $profit = $cartSubtotal - $productBuyingSubtotal;
        if ($profit < 0) {
            $loss = abs($profit);
            $profit = 0;
        } elseif ($profit == 0) {
